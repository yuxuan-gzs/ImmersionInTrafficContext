...
public class Jei4Recipe implements Recipe<RecipeInput> {
	private final List<ItemStack> output;
	private final NonNullList<Ingredient> recipeItems;
	private final List<Integer> integers;

	public Jei4Recipe(List<ItemStack> output, NonNullList<Ingredient> recipeItems, List<Integer> integers) {
		this.output = output;
	...
	public static class Type implements RecipeType<Jei4Recipe> {
		private Type() {
		}

		public static final RecipeType<Jei4Recipe> INSTANCE = new Type();
	}

	public static class Serializer implements RecipeSerializer<Jei4Recipe> {
		public static final Serializer INSTANCE = new Serializer();
		private static final MapCodec<Jei4Recipe> CODEC = RecordCodecBuilder
				...
				}, DataResult::success).forGetter(recipe -> recipe.recipeItems), Codec.INT.listOf().fieldOf("integers").forGetter(recipe -> recipe.integers)).apply(builder, Jei4Recipe::new));
		public static final StreamCodec<RegistryFriendlyByteBuf, Jei4Recipe> STREAM_CODEC = StreamCodec.of(Serializer::toNetwork, Serializer::fromNetwork);

		@Override
		public MapCodec<Jei4Recipe> codec() {
			return CODEC;
		}

		@Override
		public StreamCodec<RegistryFriendlyByteBuf, Jei4Recipe> streamCodec() {
			return STREAM_CODEC;
		}

		private static Jei4Recipe fromNetwork(RegistryFriendlyByteBuf buf) {
			...
			return new Jei4Recipe(outputs, inputs, numbers);
		}

		private static void toNetwork(RegistryFriendlyByteBuf buf, Jei4Recipe recipe) {
