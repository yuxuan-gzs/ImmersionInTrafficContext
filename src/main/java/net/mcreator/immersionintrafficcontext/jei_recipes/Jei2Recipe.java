...
public class Jei2Recipe implements Recipe<RecipeInput> {
	private final List<ItemStack> output;
	private final NonNullList<Ingredient> recipeItems;
	private final List<Integer> integers;

	public Jei2Recipe(List<ItemStack> output, NonNullList<Ingredient> recipeItems, List<Integer> integers) {
		this.output = output;
	...
	public static class Type implements RecipeType<Jei2Recipe> {
		private Type() {
		}

		public static final RecipeType<Jei2Recipe> INSTANCE = new Type();
	}

	public static class Serializer implements RecipeSerializer<Jei2Recipe> {
		public static final Serializer INSTANCE = new Serializer();
		private static final MapCodec<Jei2Recipe> CODEC = RecordCodecBuilder
				...
				}, DataResult::success).forGetter(recipe -> recipe.recipeItems), Codec.INT.listOf().fieldOf("integers").forGetter(recipe -> recipe.integers)).apply(builder, Jei2Recipe::new));
		public static final StreamCodec<RegistryFriendlyByteBuf, Jei2Recipe> STREAM_CODEC = StreamCodec.of(Serializer::toNetwork, Serializer::fromNetwork);

		@Override
		public MapCodec<Jei2Recipe> codec() {
			return CODEC;
		}

		@Override
		public StreamCodec<RegistryFriendlyByteBuf, Jei2Recipe> streamCodec() {
			return STREAM_CODEC;
		}

		private static Jei2Recipe fromNetwork(RegistryFriendlyByteBuf buf) {
			...
			return new Jei2Recipe(outputs, inputs, numbers);
		}

		private static void toNetwork(RegistryFriendlyByteBuf buf, Jei2Recipe recipe) {
