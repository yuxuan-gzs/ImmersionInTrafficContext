...
public class Jei1Recipe implements Recipe<RecipeInput> {
	private final List<ItemStack> output;
	private final NonNullList<Ingredient> recipeItems;
	private final List<Integer> integers;

	public Jei1Recipe(List<ItemStack> output, NonNullList<Ingredient> recipeItems, List<Integer> integers) {
		this.output = output;
	...
	public static class Type implements RecipeType<Jei1Recipe> {
		private Type() {
		}

		public static final RecipeType<Jei1Recipe> INSTANCE = new Type();
	}

	public static class Serializer implements RecipeSerializer<Jei1Recipe> {
		public static final Serializer INSTANCE = new Serializer();
		private static final MapCodec<Jei1Recipe> CODEC = RecordCodecBuilder
				...
				}, DataResult::success).forGetter(recipe -> recipe.recipeItems), Codec.INT.listOf().fieldOf("integers").forGetter(recipe -> recipe.integers)).apply(builder, Jei1Recipe::new));
		public static final StreamCodec<RegistryFriendlyByteBuf, Jei1Recipe> STREAM_CODEC = StreamCodec.of(Serializer::toNetwork, Serializer::fromNetwork);

		@Override
		public MapCodec<Jei1Recipe> codec() {
			return CODEC;
		}

		@Override
		public StreamCodec<RegistryFriendlyByteBuf, Jei1Recipe> streamCodec() {
			return STREAM_CODEC;
		}

		private static Jei1Recipe fromNetwork(RegistryFriendlyByteBuf buf) {
			...
			return new Jei1Recipe(outputs, inputs, numbers);
		}

		private static void toNetwork(RegistryFriendlyByteBuf buf, Jei1Recipe recipe) {
